...
# /// script
# dependencies = [
#     "transformers @ git+https://gitclone.com/github.com/huggingface/transformers.git",
#     "accelerate >= 0.12.0",
#     "datasets >= 1.8.0",
...
os.environ["DATASETS_ENDPOINT"] = "https://hf-mirror.com"
...
class DataTrainingArguments:
    ...
    early_stopping_patience: Optional[int] = field(
        default=None,
        metadata={"help": "如果你想开启早停，请设置耐心值（例如 3 或 5）。不设置则不开启早停。"}
    )
    clean_text: bool = field(
        default=False,
        metadata={
            "help": (
                "Whether to apply a light text cleaning step (HTML unescape, strip simple tags, collapse whitespace)."
            )
        },
    )
    lowercase_text: bool = field(
        default=False,
        metadata={"help": "Whether to convert the input text to lowercase before tokenization."},
    )
    ...
    def create_optimizer(self):
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters = [
                {"params": [p for _, p in other_params],      "lr": self.args.learning_rate, "names": [n for n, _ in other_params]},
                {"params": [p for _, p in classifier_params], "lr": 1e-4,                     "names": [n for n, _ in classifier_params]},
            ]
            self.optimizer = torch.optim.AdamW([{k: v for k, v in group.items() if k != "names"} for group in optimizer_grouped_parameters])

            print("Base lr:", self.args.learning_rate)
...
def main():
    ...
    def preprocess_function(examples):
        ...
        if data_args.clean_text or data_args.lowercase_text:
            normalized_sentences = []
            for sentence in sentences:
                sentence = "" if sentence is None else sentence
                if data_args.clean_text:
                    sentence = _basic_clean_text(sentence)
                else:
                    sentence = sentence.strip()
                if data_args.lowercase_text:
                    sentence = sentence.lower()
                normalized_sentences.append(sentence)
        else:
            normalized_sentences = [
                sentence.strip() if isinstance(sentence, str) else "" for sentence in sentences
            ]
        # Tokenize the texts
        result = tokenizer(normalized_sentences, padding=padding, max_length=max_seq_length, truncation=True)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # 1. 计算预测类别 (Hard Predictions) -> 用于 Accuracy 和 F1
        preds = np.argmax(logits, axis=1)
        
        # 2. 计算概率 (Probabilities) -> 用于 ROC-AUC
        # Logits 经过 Softmax 变成概率
        ...
        # F1-Score
        # 注意：如果是二分类用 'binary'，多分类建议用 'weighted' 或 'macro'
        # 这里假设是二分类或需要加权的多分类，使用 'weighted' 比较通用
        ...
        try:
            if len(np.unique(labels)) == 2:
                # 二分类，通常取正类（index 1）的概率
                roc_auc = roc_auc_score(labels, probs[:, 1])
            else:
                # 多分类，需要指定 ovr (One-vs-Rest)
                roc_auc = roc_auc_score(labels, probs, multi_class='ovr', average='weighted')
        except ValueError:
            # 如果验证集中某个类别缺失，roc_auc可能会报错，设为 None 或 0
            roc_auc = 0.0
    ...
    # === 新增逻辑：根据参数决定是否添加 Callback ===
    trainer_callbacks = []
    if data_args.early_stopping_patience is not None:
        logger.info(f"开启早停机制 (Early Stopping)，耐心值: {data_args.early_stopping_patience}")
        trainer_callbacks.append(EarlyStoppingCallback(early_stopping_patience=data_args.early_stopping_patience))
